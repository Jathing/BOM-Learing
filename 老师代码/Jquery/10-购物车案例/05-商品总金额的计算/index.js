...
$(function(){
    ...
    $('.pnum_reduce').click(function(){
        ...
        changeXJ($(this).parents('.item').index());
        // 调用商品总和
        finallyP()
    })
    ...
    function changeXJ(index){
        ...
        $('.total_price').eq(index).html(moneyAll + '元')
    }
    ...
    function finallyP(){
        ...
        $('.item_option').each(function(index,item){
            if($(item).prop('checked')){
                ...
                priSum += parseInt($('.total_price').eq(index).html());

                // 拿到总数量和总金额 对两个变量重新进行赋值
                $('.total_num b').html(numSum);
                $('.sum_price b').html(priSum);
            }
        })
    }
